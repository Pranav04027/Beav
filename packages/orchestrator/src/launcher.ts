  ...
  try {
    ...
    const child = spawn('bash', [workerScript], {
      ...
      env: {
        ...
        DB_PATH: path.resolve(process.cwd(), '../../beav.db'),
      },
      detached: false,
    });
    ...
    if (child.pid) {
      await db
        ...
        .where(eq(tasks.id, task.id));
      console.log(`[Launcher] Task ${task.id} started with PID: ${child.pid}`);
    }
    ...
    child.on('close', async (code) => {
      ...
      if (code !== 0) {
        ...
        console.error(`[Launcher] Task ${task.id} crashed with code ${code}`);
      } else {
        console.log(
